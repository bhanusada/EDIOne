...
@Component({
    ...
    styleUrls: ['./potype.component.scss']
})
export class PotypeComponent implements OnInit {

    @ViewChild('poChart') poChart: BaseChartDirective;
    ...
    private refresh() {
        this.serverService.poRefresh().subscribe(message => {
            console.log(message);
            this.sortMetrics();
        });
    }
    ...
    public sortMetrics() {
        this.lineChartData.sort((a, b) =>
            b.data.reduce( (acc, cur) => acc = acc + cur) - a.data.reduce( (acc, cur) => acc = acc + cur)
        )
    }
